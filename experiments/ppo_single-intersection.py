...
def make_env(
    ...
    # SumoEnvironment enforces: delta_time > yellow_time.
    # For 1s control step (delta_time=1), we disable yellow to satisfy the constraint.
...
def main() -> None:
    prs = argparse.ArgumentParser(
        ...
        description="PPO training on SUMO-RL single-intersection (traffic signal control).",
    )
    prs.add_argument(
        "--route",
        type=str,
    ...
    prs.add_argument("--lr", type=float, default=3e-4, help="Learning rate.")
    ...
    prs.add_argument("--hidden", type=int, nargs=2, default=[64, 64], help="MLP hidden sizes.")
    ...
    prs.add_argument(
        "--logdir",
        type=str,
        default="logs/ppo_single_intersection",
        help="Tensorboard log dir.",
    )
    prs.add_argument(
        "--monitor-dir",
        type=str,
        default="logs/ppo_single_intersection/monitor",
        help="Directory to write Monitor logs (for reward curve plotting).",
    )
    prs.add_argument(
        "--model-out",
        type=str,
        default="models/ppo_single_intersection.zip",
    ...
    out_csv = f"outputs/single-intersection/ppo_train/{experiment_time}"
